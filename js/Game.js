class Game {
  ...
  play(){
    ...
    if(player.distance>5000 && passed===false){
       passed=true;
  ...
  end()
  {
   ...
   for(var plr in allPlayers )
   {
     if(allPlayers[plr].rank===1){
       fill("golden")
      text("1st :  " +allPlayers[plr].name,displayWidth/2,200)
     }
     if(allPlayers[plr].rank===2){
       fill("silver")
      text("2nd :  " +allPlayers[plr].name,displayWidth/2,250)
     }
     if(allPlayers[plr].rank===3){
       fill("bronze")
      text("3rd :  " +allPlayers[plr].name,displayWidth/2,300)
     }
     if(allPlayers[plr].rank===4){
       fill("red")
      text("4th :  " +allPlayers[plr].name,displayWidth/2,350)
     }
